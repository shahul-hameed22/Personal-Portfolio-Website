...
const NavBar = (props) => {
    ...
    const lightModeStyle = "text-white border-b-2 border-purple-600 hover:border-purple-900 px-3  py-1 text-md font-medium"
    const darkModeStyle = "text-white border-b-2 border-gray-500 hover:border-gray-600 px-3  py-1 text-md font-medium"


    return (
        <nav className={`navbar navbar-expand-lg navbar-light fixed-top p-3 bg-gradient-to-b ${props.mode === "light" ? 'from-purple-800 to-purple-300' : 'from-black  to-gray-800 '}`}>
            <div className="container-fluid">
                <button className={`navbar-toggler px-3 py-1 ${props.mode === "dark" && 'bg-info'}`} type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul className="navbar-nav me-auto mb-2 mb-lg-0 gap-2">
                        {props.mode === "light" ?
                            navLink.map((i) => {
                                return (
                                    <a className={`${lightModeStyle} ${Active === i.href && 'border-purple-900 border-b-4'} `} onClick={() => { handleActive(i.href) }} aria-current="page" href={i.href}>{i.text}</a>
                                )
                            }) :
                            navLink.map((i) => {
                                return (
                                    <a className={`${darkModeStyle} ${Active === i.href && 'border-info border-b-4'} `} onClick={() => { handleActive(i.href) }} aria-current="page" href={i.href}>{i.text}</a>
                                )
                            })
                        }
                    </ul>
                </div>
                <input onChange={props.DarkMode} type="checkbox" id="darkmode-toggle" checked={props.isChecked} />
                <label for="darkmode-toggle">
                    <svg version="1.1" class="sun" xmlns="http://www.w3.org/2000/svg" xmlnsxlink="http://www.w3.org/1999/xlink"
                        x="0px" y="0px" viewBox="0 0 496 496" style={{ enableBackground: "new 0 0 496 496" }} xmlspace="preserve" >
				...
				c-79.4,0-144-64.6-144-144s64.6-144,144-144s144,64.6,144,144S327.4,392,248,392z" />
                    ...
                    <svg version="1.1" class="moon" xmlns="http://www.w3.org/2000/svg" xmlnsxlink="http://www.w3.org/1999/xlink"
                        x="0px" y="0px" viewBox="0 0 49.739 49.739" style={{ enableBackground: "ackground:new 0 0 49.739 49.739" }}
                        xmlspace="preserve">
